...
def writeresult(pgdict, video_path, video_save_folder, IDENTITY_MAP):
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                for t in online_targets:
                    ...
                    if tid == "NOT_IN_DATABASE":
                        tid_map = "NaN"
                        colorid = 0
                    else:
                        gallery_key = tid.split("-")[0]
                        person_key = tid.split("-")[1]
                        if gallery_key in IDENTITY_MAP and person_key in IDENTITY_MAP[gallery_key]:
                            tid_map = IDENTITY_MAP[gallery_key][person_key]['name']
                        else:
                            tid_map = "Unknown"
                        colorid = int(person_key)
