...
def main():
    ...
    for filename in os.listdir(probe_folder_path):
        if filename.lower().endswith('.mp4'):
            video_path = osp.join(probe_folder_path, filename)
            probe_video.append(video_path)
    
    # probe_video = ["./InputVideos/probe/probe_0.mp4"]

    for probe_video_path in probe_video:
        # tracking
        ...
        for probe_id, result in gallery_probe_result.items():
            if result == "NOT_IN_DATABASE":
                ...
                print("distance exceeds the threshold.")
                print("=================================")
                continue
            ...
            if '-' in result:
                ...
                if gallery_name in IDENTITY_MAP and person_id in IDENTITY_MAP[gallery_name]:
                    identity_info = IDENTITY_MAP[gallery_name][person_id]
                    print ("=================================")
                    ...
                    print("=================================")
                    print(f"Probe ID: {probe_id} -> {result}")
                    print(f"not found in identity map (Gallery: {gallery_name}, Person: {person_id})")
                    print("=================================")
